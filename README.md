...
Welcome! This repository is the starting point for your first web development assignment.
...
> **Note:** You will know you are successful when the top-left of the page says:  
...
4.  Run the following commands (paste your link where indicated):

```bash
git clone <PASTE_YOUR_REPO_LINK_HERE>
cd intro-web-starter
```
...
2.  **`style.css`**: Change the background color, font styles, and button appearance.
3.  **`script.js`**: Add a function to make the button alert a message when clicked.
...
1.  **Edit `index.html`** → Save → Run:
    ...
    ```
2.  **Edit `style.css`** → Save → Run:
    ```bash
    git add style.css
    git commit -m "Add CSS styling"
    ```
3.  **Edit `script.js`** → Save → Run:
    ```bash
    git add script.js
...
4.  Copy your repository URL:
    > `https://github.com/your-username/intro-web-starter`
5.  Submit this link in the LMS.

**❌ Do NOT submit:** ZIP files, Google Docs, or Screenshots.
